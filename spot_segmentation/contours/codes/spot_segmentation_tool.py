...
class SpotSegmentor:
    ...
    def get_closed_contours(self):
        ...
        for level, path in zip(self.contours.levels, self.contours.get_paths()):
            ...
            if all_codes_arr is None:
                continue
            start_ind = np.where(all_codes_arr==1)[0]
            for i in range(len(start_ind)): 
                if i == len(start_ind)-1:
                    if all_codes_arr[-1]==79:
    ...
    def get_segmentation_dict(self):  
        """
        This function links overlapping contours and saves the base contour
        of each contour set in a dictionary. We assume that there is a homgenous 
        Atga8a "background" intensity level that separates all spots. SInce this
        background level will form a large contour, it should have been filtered
        in the max_area parameter. If one base contour contains several peaks, 
        it is because the Atga8a vesicles are 
        colocalised and should not be separated as this would shift the base contour
        upwards and therefore reduce the base contour area. If instead each peaks is
        to be found, the get_peaks_dict() function should be used.
        """
        mask =  np.zeros_like(self.image, dtype=np.uint8)

        paths = self.closed_paths.copy()

        # Make mesh grid for getting contour pixels in the loop
        height = mask.shape[0]
        width = mask.shape[1]
        y, x = np.mgrid[:height, :width]
        points = np.column_stack((x.ravel(), y.ravel()))

        # The temporary path is sorted on area so that we start with lowest contour
        ...
        while len(paths_temp)>0:
            ...
            if is_large:         
                ...
                new_mask = base_path.contains_points(points).reshape(height, width)
                mask[new_mask] = 1
