...
a gt mask and FP is any mask outside of gt mask. FP is any mask that does
not contain a detected peak.
...
results_dir = '../results/results_fullsize'
for file in os.listdir(results_dir):
    ...
    masks_uint8 = cv2.imread(f'../../data/full_size/test/masks/{file[:-4]}_masks.png', cv2.IMREAD_UNCHANGED)
    masks = masks_uint8/255

    # filter low elevation and prominence
    ...
    print(f'tp: {int(tp)}/{value}')
    print(f'fn: {int(fn)}/{value}')
    print(f'fp: {int(fp)}')
    ...
    for index, row in true_df.iterrows():
        plt.plot(row['elevation'], row['prominence'], marker='.', linestyle='None', markersize=4, c='r', alpha=0.1)
    for index, row in false_df.iterrows():
        plt.plot(row['elevation'], row['prominence'], marker='.', linestyle='None', markersize=4, c='b', alpha=0.1)
    ...
    if 'R1h' in file:
        plt.xlim(0,3500)
        plt.ylim(0, 3500)
        plt.title('Refed Cells')
    elif 'S4h' in file:
        plt.title('Starved Cells')
        plt.xlim(0, 3500)
        plt.ylim(0, 3500)
    elif 'F' in file:
        plt.title('Fed Cells')
    ...
    red_marker = mlines.Line2D([], [], color='r', marker='.', linestyle='None', markersize=4, label='True Positive')
    blue_marker = mlines.Line2D([], [], color='b', marker='.', linestyle='None', markersize=4, label='False Positive')

# Add legend with custom handles
    plt.legend(handles=[red_marker, blue_marker], loc='upper left')
    plt.grid()
    plt.tight_layout()
    plt.savefig(f'../plots/{file[:-4]}_prom_vs_el.png')
